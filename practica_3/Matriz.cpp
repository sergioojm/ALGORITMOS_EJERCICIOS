...
/*
  ...
  Numero de columnas y el numero de filas deben de ser iguales mayores a 0

  Complejidad temporal O: O(n!)?
*/

double Matriz::calcularDeterminante()
{
  
  assertdomjudge(this->n_filas > 0);
  assertdomjudge(this->n_columnas > 0);
  assertdomjudge(this->n_columnas == this->n_filas);

  if (this->n_columnas == 1)
  {
  ...
  if (this->n_columnas == 2)
  {
    return ((this->matriz[0][0] * this->matriz[1][1]) -
            (this->matriz[0][1] * this->matriz[1][0])
            ); 
  }
  ...
  for (int i = 0; i < this->n_filas; i++)
  {
    ...
    Matriz menor = this->obtenerMenor(0, i);

    resultado += pow(-1, i) * this->matriz[0][i] * menor.calcularDeterminante();
...
Matriz Matriz::obtenerMenor(int fila, int columna)
{
    Matriz menor(this->n_filas - 1, this->n_columnas - 1);
    int menorFila = 0, menorColumna = 0;
    ...
    for (int i = 0; i < this->n_filas; i++)
    {
        if (i != fila)
        {
          ...
          for (int j = 0; j < this->n_columnas; j++)
          {
              if (j != columna)
              {
                menor.matriz[menorFila][menorColumna] = this->matriz[i][j];
                menorColumna++;
              }
          }
          menorFila++;
